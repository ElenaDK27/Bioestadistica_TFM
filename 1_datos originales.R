...
for (i in 6:27) {
  ...
  p_valores[i - 5] <- coef(summary(modelo))["SexoMujer", "Pr(>|z|)"]
}
...
datos_bayes<-cbind(datos_bayes$sexo,datos_transformados[,6:27])
...
save(modelo_P1, modelo_P2, modelo_P3, modelo_P4, modelo_P5, modelo_P6, modelo_P7, modelo_P8, modelo_P9, modelo_P10, modelo_P11, modelo_P12, modelo_P13, modelo_P14, modelo_P15, modelo_P16, modelo_P17, modelo_P18, modelo_P19, modelo_P20, modelo_P21, modelo_P22, file = "modelos_brms.RData")

load("modelos_brms.RData")
...
for (i in 1:length(modelos)) {
  resumen <- summary(modelos[[i]])$fixed[5, c(1, 3, 4)] # Asegurarse de que el índice sea correcto
...
which(sem_bueno$pvalue<0.05) #1  2  3 11 12 19 20 22

summary(modelo_sem)
bloque1<-lavPredict(modelo_sem,newdata=datos)
mean(bloque1[,1])
#COMO SACAR LA VARIABLE LATENTE

parameterestimates(modelo_sem)[c(23,45:48),]
lavInspect(modelo_sem, "h1")
lavPredict(modelo_sem)


#prueba de mann whitney ##########################################
datos[,6:27]<-lapply(datos[,6:27],as.numeric)
...
for (i in 6:27) {  
  mw_results[[i-5]] <- perform_mann_whitney(datos, names(datos)[i])
}
