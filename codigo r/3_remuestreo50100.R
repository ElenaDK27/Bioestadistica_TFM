...
prueba <- data.frame(sexo = rep(c("hombre", "mujer"), c(50, 100)))
num_preguntas <- 22
ajustar_longitud_respuestas <- function(respuestas, total) {
  if (length(respuestas) > total) {
    respuestas <- respuestas[1:total]
  } else if (length(respuestas) < total) {
    respuestas <- c(respuestas, sample(respuestas, total - length(respuestas), replace = TRUE))
  }
  return(respuestas)
}
...
for (i in 1:num_preguntas) {
  ...
  n_respuestas_por_categoria_hombres <- round(50 * proporciones$prop[inicio_hombres:fin_hombres])
  n_respuestas_por_categoria_mujeres <- round(100 * proporciones$prop[inicio_mujeres:fin_mujeres])
  ...
  respuestas_hombres <- ajustar_longitud_respuestas(respuestas_hombres, 50)
  respuestas_mujeres <- ajustar_longitud_respuestas(respuestas_mujeres, 100)
...
head(prueba, 10)
tail(prueba, 10)
datos <- data.frame(lapply(prueba, factor))
...
which(sem_bueno$pvalue<0.05) #1 y 3
...
save(modelo_P1, modelo_P2, modelo_P3, modelo_P4, modelo_P5, modelo_P6, modelo_P7, modelo_P8, modelo_P9, modelo_P10, modelo_P11, modelo_P12, modelo_P13, modelo_P14, modelo_P15, modelo_P16, modelo_P17, modelo_P18, modelo_P19, modelo_P20, modelo_P21, modelo_P22, file = "modelos_brms_remuestreo2.RData")

load("modelos_brms_remuestreo2.RData")

resultados <- data.frame(
  modelo = character(23),
  estimacion = numeric(23),
  IC_inferior = numeric(23),
  IC_superior = numeric(23),
  stringsAsFactors = FALSE
)

modelos <- list(modelo_P1, modelo_P2, modelo_P3, modelo_P4, modelo_P5, modelo_P6, modelo_P7, modelo_P8, modelo_P9, modelo_P10, modelo_P11, modelo_P12, modelo_P13, modelo_P14, modelo_P15, modelo_P16, modelo_P17, modelo_P18, modelo_P19, modelo_P20, modelo_P21, modelo_P22,modelo_P23)
